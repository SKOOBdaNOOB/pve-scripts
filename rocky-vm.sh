...
ROCKY_VERSION="9"
ROCKY_URL="https://dl.rockylinux.org/pub/rocky/${ROCKY_VERSION}/images/x86_64/Rocky-${ROCKY_VERSION}-GenericCloud-Base.latest.x86_64.qcow2"
DISK_SIZE="32G"
...
function default_settings() {
  ...
  echo -e "${YELLOW}Using Rocky Linux Version: ${BLUE}${ROCKY_VERSION}${RESET}"
  ...
  echo -e "${BLUE}Creating a Rocky Linux VM using the above default settings${RESET}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 rocky-vm --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="rocky-vm"
      echo -e "${YELLOW}Using Hostname: ${BLUE}$HN${RESET}"
    else
  ...
  if CI_USERNAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Cloud-Init Username" 8 58 rocky --title "CLOUD-INIT USERNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create Rocky Linux VM?" --no-button Do-Over 10 58); then
    echo -e "${RED}Creating a Rocky Linux VM using the above advanced settings${RESET}"
  else
    header_info
...
msg_info "Retrieving Rocky Linux ${ROCKY_VERSION} Cloud Image"
URL=$ROCKY_URL
wget -q --show-progress $URL -O $TEMP_DIR/rocky-cloud.qcow2
echo -en "\e[1A\e[0K"
msg_ok "Downloaded ${RESET}${BLUE}Rocky-${ROCKY_VERSION}-GenericCloud-Base.qcow2${RESET}"
...
msg_info "Creating Rocky Linux VM"
qm create $VMID -agent 1${MACHINE} -tablet 0 -localtime 1 -bios ovmf${CPU_TYPE} -cores $CORE_COUNT -memory $RAM_SIZE \
  -name $HN -tags rocky-linux -net0 virtio,bridge=$BRG,macaddr=$MAC$VLAN$MTU -onboot 1 -ostype l26 -scsihw virtio-scsi-pci
...
qm importdisk $VMID $TEMP_DIR/rocky-cloud.qcow2 $STORAGE ${DISK_IMPORT:-} 1>&/dev/null
...
qm set $VMID \
  ...
  -description "Rocky Linux ${ROCKY_VERSION} VM created by script" >/dev/null
...
msg_ok "Created Rocky Linux VM ${RESET}${BLUE}(${HN})${RESET}"

if [ "$START_VM" == "yes" ]; then
  msg_info "Starting Rocky Linux VM"
  qm start $VMID
  msg_ok "Started Rocky Linux VM"
fi
